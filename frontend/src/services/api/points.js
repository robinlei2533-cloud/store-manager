...
export async function getPointsRules() {
  ensureLocalInit();
  if (isLocal()) return localDb.all('fan_points_rules').sort((a, b) => b.points - a.points);
...
export async function createPointsRule(rule) {
  ensureLocalInit();
  if (isLocal()) return localDb.insert('fan_points_rules', rule);
...
export async function updatePointsRule(id, rule) {
  ensureLocalInit();
  if (isLocal()) return localDb.update('fan_points_rules', id, rule);
...
export async function deletePointsRule(id) {
  ensureLocalInit();
  if (isLocal()) { localDb.remove('fan_points_rules', id); return; }
...
export async function getLevelRules() {
  ensureLocalInit();
  if (isLocal()) return localDb.all('fan_level_rules').sort((a, b) => a.min_points - b.min_points);
...
export async function updateLevelRule(id, rule) {
  ensureLocalInit();
  if (isLocal()) return localDb.update('fan_level_rules', id, rule);
